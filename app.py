...
# إعداد الصفحة لتظهر بشكل عريض ومنظم
st.set_page_config(page_title="نظام وقود المكاتب", layout="wide")

# كود CSS و JavaScript لإضافة وظيفة الطباعة وإخفاء العناصر غير الضرورية عند الطباعة
def add_print_button():
st.markdown(
"""
<style>
@media print {
/* إخفاء القائمة الجانبية والأزرار ورأس الصفحة عند الطباعة */
...
/* تنسيق الجدول ليظهر بشكل رسمي في الورقة */
table {
width: 100% !important;
...
st.write("قم بتعبئة البيانات من القائمة الجانبية للحصول على الجدول النهائي.")

# --- قسم مدخلات البيانات في القائمة الجانبية ---
with st.sidebar:
st.header("📋 مدخلات التقرير")

# 1. المكاتب (يمكنك تعديل هذه القائمة لاحقاً بأسماء مكاتبكم)
...
# 3. سعر اللتر (متغير ولا يظهر في الجدول النهائي)
...
bons_input = st.text_area("أدخل المبالغ (افصل بينها بمسافة):", help="مثال: 2000 1500 3000")

# --- العمليات الحسابية والعرض ---
# يتم التحديث تلقائياً بمجرد إدخال البيانات
if index_fin > 0:
try:
# حساب إجمالي البونات
...
# حساب الرصيد المتبقي نهاية الشهر
reste_31 = solde_init + chargement - total_consom_da

# حساب متوسط الاستهلاك (L/100KM)
if km_parcourus > 0:
...
# بناء الجدول النهائي (بدون سعر اللتر كما طلبت)
...
st.table(df) # استخدام st.table لأنه يظهر بشكل أفضل في الطباعة

if km_parcourus < 0:
st.error("⚠️ تنبيه: العداد الحالي أقل من العداد السابق، يرجى مراجعة البيانات.")

except ValueError:
st.warning("الرجاء إدخال أرقام صحيحة في خانة مبالغ البونات.")
else:
st.info("💡 بانتظار إدخال قراءات العداد لإصدار التقرير.")
